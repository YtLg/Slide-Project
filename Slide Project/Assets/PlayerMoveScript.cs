...
public class Rigid : MonoBehaviour
{
    ...
    void Update(){
        ...
        if (Input.GetMouseButtonDown(0)){
            ...
            if (xMousePos > maxForce){
                newForceMouse = new Vector3(maxForce, worldMousePos.y, 0);
            }
            else if(yMousepos > maxForce){
                newForceMouse = new Vector3(worldMousePos.x, maxForce, 0);
            }
            else if (xMousePos > maxForce && yMousepos > maxForce)
            {
